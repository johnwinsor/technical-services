...
"""
Workday CSV to Alma PO Line JSON Converter
Converts Workday purchase order CSV files to Alma library system PO line JSON format
"""
...
def parse_csv_row(row: Dict[str, Any]) -> Optional[POLineData]:
    ...
    title = str(row.get('Item Description', '')).strip()
    asin = clean_asin(row.get('Supplier Item Identifier', ''))
    ...
    return POLineData(
        title=title,
        order_id=str(row.get('Supplier Item Identifier', '')).strip(),
        asin=asin,
...
def get_user_metadata(data: POLineData, alma_config: Optional[AlmaConfig]) -> POLineData:
    ...
    data.author = questionary.text(
        "Author:",
        default=oclc_data.get('author', '')
    ).ask()
    
    data.isbn = questionary.text(
        "ISBN:",
        default=oclc_data.get('isbn', '')
    ).ask()
    
    data.publisher = questionary.text(
        "Publisher:",
        default=oclc_data.get('publisher', '')
    ).ask()
    
    data.publication_year = questionary.text(
        "Publication year:",
